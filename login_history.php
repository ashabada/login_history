...
class login_history extends rcube_plugin
	...
	function login_info($p)
		...
		$username = $user->data['username'];
		return array(
			'content' => rcube::Q($username) . " (" . rcube::Q($this->gettext('lastlogin')) . '<a href="./?_action=plugin.login_history" class="about-link">' . rcube::Q($_SESSION['remoteip']) . " " . rcube::Q($_SESSION['logintime']) . "</a>" . ")"
		);
		}
	...
	function showinfo()
		...
		$geoip_db = $rcmail->config->get('login_history_geoip_db');
		...
		if ($enable_geoip == '1')
			...
			while (($data = $rcmail->get_dbh()->fetch_assoc($res)))
				...
				try
					{
					$record = $reader->country($data['remoteip']);
				...
				$table->add('', "<img src=\"plugins/login_history/img/blank.gif\" class=\"flag flag-" . $country_code . "\" alt=\"" . $country_code . "\" />", $mode = 'strict');
				}
